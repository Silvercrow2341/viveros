...
def procesar_pedido(request):
    ...
    if request.method == 'POST':
        ...
        if user_profile_form.is_valid():
            ...
            carro=Carro(request)     #Recupera el carrito de compras del usuario (carro) y procesa su contenido para crear objetos LineaPedido.
    ...
    return render(request, 'emails/form.html', {'user_profile_form': user_profile_form})

def enviar_mail(**kwargs):
    asunto="Gracias por el pedido"
    mensaje=render_to_string("emails/pago.html",{
...
def portafolio(request):
    redes=links.objects.all()
    projects=productos.objects.all()
    return  render(request,"pedidos/form.html",{'projects':projects,'redes':redes})